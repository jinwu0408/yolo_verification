...
class StreamingExample(threading.Thread):
    ...
    def show_yuv_frame(self, window_name, yuv_frame):
        ...
        info = yuv_frame.info()
        # print('yuv_frameyuv_frameyuv_frameyuv_frameyuv_frameyuv_frameyuv_frame')
        ...
        cv2frame = detect_drone(cv2frame)


        # cv2frame = cv2.rectangle(im0, start_point,end_point,color=(255,0,0),thickness =2)
        # cv2.imshow(window_name, cv2frame)

        if cv2.waitKey(1)==ord('0'):
            cv2.imwrite('./tmp_img_dir/img{}.jpg'.format(self.img_saved),cv2frame)
            self.img_saved +=1
            print('{} images_data_collected'.format(self.img_saved))
