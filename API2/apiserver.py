...
class get_Drone_Task(Resource):
    def get(self,drone_id):
        ...
        query = conn.execute("select task from tasks where DroneId ="+ drone_id +" order by taskid")
        result = {'data': [dict(zip(tuple (query.keys()) ,i)) for i in query.cursor]}
	#result=str(query.keys()).split(",")
        return result
#jsonify(result)
#            imagepath1=str(image_file).split("'")
...
api.add_resource(get_Drone_Task, '/get_Drone_Task/<drone_id>')
